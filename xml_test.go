...
func TestWriteXMLFilenameIsRequired(t *testing.T) {
	if err := WriteXML("", struct{}{}); err == nil {
...
func TestWriteXML(t *testing.T) {
	...
	tests := []struct {
		...
		{
			Input:    testGroup{},
			Expected: []byte("<testGroup><GroupName></GroupName></testGroup>"),
		},
		{
			Input:    testData,
			Expected: []byte(`<testGroup><GroupName>TheGophers</GroupName><Members><Name>Alice</Name><Age>19</Age></Members><Members><Name>Bob</Name><Age>52</Age></Members></testGroup>`),
		},
	}

	filename := filepath.Join(dir, "data.xml")
	for _, test := range tests {
		err := WriteXML(filename, &test.Input)
		if err != nil {
			t.Fatal(err)
		}
...
func TestReadXMLFilenameIsRequired(t *testing.T) {
	if err := ReadXML("", nil); err == nil {
